...
const Platform=({pjs,setPjs,sv,initialProject,onClearInitial})=>{
  ...
  // CHANGE 2: Auto-feed mix producto â†’ producto (tamApto + parqApto)
    ...
    <div>
      ...
      {vw==="list"&&(
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
          ...
          {pjs.length===0
            ? <div className="text-center py-16 bg-white rounded-2xl border border-slate-200"><div className="text-4xl mb-3 opacity-30">ðŸ“‹</div><div className="text-sm text-slate-400 mb-3">Sin proyectos</div><button onClick={newP} className="px-4 py-2 bg-emerald-500 text-white font-semibold rounded-lg text-sm">Crear anÃ¡lisis</button></div>
                ...
                return (
                  <div key={p.id} className="bg-white rounded-xl border border-slate-200 px-5 py-4 flex items-center justify-between hover:shadow-md cursor-pointer group" onClick={()=>loadP(p)}>
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-slate-400 mt-0.5">{p.inputs.ubicacion||""}{p.inputs.equipo?' Â· '+p.inputs.equipo:''} Â· {new Date(p.at).toLocaleDateString()}</div>
                      {r&&<div className="flex gap-4 mt-2">
                        <span className="text-xs"><span className="text-slate-400">TIR: </span><span className={`font-mono font-bold ${r.airrBT>0.15?"text-emerald-600":"text-red-500"}`}>{r.airrBT?fp(r.airrBT):"â€”"}</span></span>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 mb-4">
            <div className="flex-1 flex flex-col gap-2 w-full">
              <div className="flex items-center gap-3">
                ...
                <input value={inp.ubicacion} onChange={e=>u("ubicacion",e.target.value)} placeholder="UbicaciÃ³n" className="text-sm text-slate-500 bg-transparent border-b border-transparent hover:border-slate-200 focus:border-emerald-500 outline-none py-1 w-36"/>
              </div>
              <div className="flex items-center gap-3">
                <select value={inp.equipo||''} onChange={e=>u("equipo",e.target.value)} className="text-xs text-slate-500 bg-white border border-slate-200 rounded-lg px-2 py-1.5 focus:outline-none focus:border-emerald-400">
                  ...
                  <option>Intus AvalÃºos</option>
                  <option>Intus ComercializaciÃ³n</option>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
            <div className="lg:col-span-8 space-y-4">
              ...
              {st===0&&<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Card t="Terrenos del proyecto" icon={ic.terrain} ch={<>
                  <div className="px-3 mb-2">
                    <div className="grid grid-cols-12 gap-1 text-[9px] font-bold text-slate-400 uppercase mb-1"><span className="col-span-1">#</span><span className="col-span-3">Nombre</span><span className="col-span-2 text-center">Ãrea mÂ²</span><span className="col-span-2 text-center">Ãrea vÂ²</span><span className="col-span-2 text-center">POT</span><span className="col-span-2"></span></div>
                    {(inp.terrenos||[]).map((t,i)=>{
                      ...
                      return(<div key={t.id||i} className="grid grid-cols-12 gap-1 items-center mb-1">
                        ...
                        <input type="number" className="col-span-2 px-1 py-1 border border-slate-200 rounded text-xs text-center" value={t.areaM2||''} placeholder="mÂ²" onChange={e=>{const ts=[...(inp.terrenos||[])];ts[i]={...ts[i],areaM2:+e.target.value||0};u("terrenos",ts);}}/>
                        ...
                        <div className="col-span-2 flex justify-end">{(inp.terrenos||[]).length>1&&<button onClick={()=>{const ts=(inp.terrenos||[]).filter((_,j)=>j!==i);u("terrenos",ts);}} className="px-2 py-1 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded text-xs" title="Eliminar terreno">Ã—</button>}</div>
                      </div>);
                    })}
                  ...
                  <div className="px-3 py-2 bg-slate-50 rounded-lg mx-3"><div className="flex justify-between text-xs"><span className="text-slate-500">Ãrea total</span><span className="font-bold">{f0(res?.aTerrTotal)} mÂ² / {f0((res?.aTerrTotal||0)*1.43115)} vÂ²</span></div>
                  <div className="flex justify-between text-xs mt-1"><span className="text-slate-500">Zona POT efectiva (mÃ¡s alta)</span><span className="font-bold text-emerald-600 font-mono">{effZone} â€” {ZG[effZone]?.n}</span></div></div>
                  <Sub t="ParÃ¡metros de zona"/>
                ...
                <Card t="Dimensiones y Edificabilidad" icon={ic.ruler} ch={<><In l="Frente" v={inp.frente} o={v=>u("frente",v)} sfx="m"/><In l="Fondo" v={inp.fondo} o={v=>u("fondo",v)} sfx="m"/><R l="Ãrea terreno (frente Ã— fondo)" v={`${f0(res?.aT)} mÂ²`} b a/><Sub t="Ãndice de edificabilidad"/><div className="px-3 mb-2"><label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={inp.useIECalc||false} onChange={e=>u("useIECalc",e.target.checked)} className="accent-emerald-500"/><span className="text-xs text-slate-500">Calcular IE desde incentivos (pestaÃ±a Edificabilidad)</span></label></div>{!(inp.useIECalc)&&<In l="Ãndice manual" v={inp.edificabilidad} o={v=>u("edificabilidad",v)}/>}{inp.useIECalc&&<R l={iauActive?"IE Aumentado":"IE Ampliado"} v={`${f2(effectiveIE)} ${iauActive?'(aumentado)':'('+peResult.pe.toFixed(1)+' PE)'}`} b a/>}<In l="Permeabilidad" v={inp.perm} o={v=>u("perm",v)} opts={["Si","No"]}/>{permFromInc&&<div className="px-3 -mt-1 mb-1"><span className="text-[9px] text-emerald-600">â† Incentivo O-5 activo ({permPct}%)</span></div>}<R l="Ãrea edificable" v={`${f0(res?.aE)} mÂ²`} b a/><Sub t="Huella del edificio"/><In l="Huella edificio" v={inp.huellaEdificio} o={v=>u("huellaEdificio",v)} sfx="mÂ²" ph="0=auto"/><R l="Niveles" v={f0(res?.niveles)} b a/><In l="Altura por losa" v={inp.alturaLosa} o={v=>u("alturaLosa",v)} sfx="m" step="0.1"/><R l="Altura total" v={`${f1(res?.alturaTotal)} m`} b a/>{inp.useIECalc&&<R l="Altura mÃ¡x. permitida" v={`${iauActive&&zData.hau?zData.hau:ieResult.ha} m`} a/>}<Sub t="Huella del sÃ³tano"/><In l="Huella sÃ³tano" v={inp.huellaSotano} o={v=>u("huellaSotano",v)} sfx="mÂ²" ph="0=auto"/><R l="SÃ³tanos" v={f2(res?.cSot)} b a/></>}/>
                <Card t="DistribuciÃ³n por Uso" icon={ic.bldg} ch={<><Sub t="% del Ã¡rea edificable"/><In l="Comercio" v={inp.pctComercio} o={v=>u("pctComercio",v)} sfx="%" step="0.5"/><In l="Oficinas" v={inp.pctOficinas} o={v=>u("pctOficinas",v)} sfx="%" step="0.5"/><R l="Apartamentos" v={`${f0(res?.pctA)}%`}/><Sub t="% vendible por uso"/><In l="Comercio" v={inp.pctVendCom} o={v=>u("pctVendCom",v)} sfx="%"/><In l="Oficinas" v={inp.pctVendOfi} o={v=>u("pctVendOfi",v)} sfx="%"/><In l="Apartamentos" v={inp.pctVendApt} o={v=>u("pctVendApt",v)} sfx="%"/><Sub t="Ãreas resultantes"/><R l="Comercio" v={`${f0(res?.aCom)} mÂ²`}/><R l="Oficinas" v={`${f0(res?.aOfi)} mÂ²`}/><R l="Aptos" v={`${f0(res?.aApt)} mÂ²`}/><R l="Total vendible" v={`${f0(res?.totalVend)} mÂ²`} b a bd/></>}/>
              </div>}
              {/* STEP 1 â€” EDIFICABILIDAD */}
              {st===1&&<div className="space-y-4">
                <Card t="Ãndice de Edificabilidad â€” Incentivos POT" icon={ic.ruler} ch={<>
                  <div className="px-3 py-3 bg-emerald-50 rounded-lg mb-3">
                    <div className="flex items-center justify-between mb-1"><span className="text-2xl font-bold text-emerald-600" style={{fontFamily:"monospace"}}>{peResult.pe.toFixed(1)} <span className="text-xs font-normal text-slate-400">PE</span></span><span className="text-xs text-slate-400">/ 100 mÃ¡x.</span></div>
                    ...
                    {new Date()<new Date('2026-12-19')&&<div className="text-[10px] text-emerald-600 mt-1">+20% bono Art. 74 Â· Base: {peResult.raw.toFixed(1)} PE</div>}
                  ...
                  <Sub t="PrÃ¡cticas incentivables â€” COM-48-2024"/>
                  {INCS.map(inc=>{
                    ...
                    return(<div key={inc.c} className={`border rounded-lg mb-1.5 overflow-hidden transition-all ${isOn?'border-emerald-200 bg-emerald-50':'border-slate-100'}`}>
                      <div className="flex items-center gap-2 px-3 py-2 cursor-pointer" onClick={inc.auto?undefined:togInc}>
                        <div className={`w-4 h-4 rounded flex items-center justify-center text-white text-[10px] ${isOn?'bg-emerald-500':'border border-slate-300'}`}>{isOn&&'âœ“'}</div>
                      ...
                      {isOn&&inc.auto&&<div className="px-3 pb-2 text-[10px] text-slate-400 italic">AutomÃ¡tico segÃºn zona â€” fijo para {effZone}</div>}
                    </div>);
                  })}
                </>}/>
                {zData.iau&&<Card t="IE Aumentado â€” AportaciÃ³n Art. 19" icon={ic.dollar} ch={<>
                  <div className="px-3 mb-2">
                    <label className="flex items-center gap-2 cursor-pointer py-2">
                      ...
                      <span className="text-xs text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full font-mono">mÃ¡x. {zData.iau}</span>
                    </label>
                    <div className="text-[10px] text-slate-400 mb-3">Permite superar el IE Ampliado ({ieResult.ie.toFixed(2)}) hasta el IE Aumentado ({zData.iau}) mediante una aportaciÃ³n econÃ³mica al municipio.</div>
                  </div>
                  {iauActive&&<>
                    <div className="px-3 py-2 bg-slate-50 rounded-lg mx-3 mb-3 text-[10px] font-mono text-slate-500" style={{lineHeight:'1.8'}}>AP = VC Ã— ((P1 âˆ’ P2) / P3) Ã— 0.5<br/>P1 = IE solicitado Â· P2 = IE ampliado actual ({ieResult.ie.toFixed(2)}) Â· P3 = IE base ({zData.ib}) Â· FOD = 0.5</div>
                    ...
                    <div className="px-3 py-2 bg-amber-50 rounded-lg mx-3 mt-2">
                      ...
                      <div className="flex justify-between text-xs mb-1"><span className="text-slate-500">IE Aumentado mÃ¡x.</span><span className="font-mono font-bold text-amber-600">{zData.iau}</span></div>
                      <div className="flex justify-between text-xs mb-1"><span className="text-slate-500">IE que se usarÃ¡</span><span className="font-mono font-bold text-amber-700">{iauIE>0?iauIE.toFixed(2):'â€”'}</span></div>
                      <div className="flex justify-between text-xs pt-2 border-t border-amber-200"><span className="font-semibold text-slate-700">AportaciÃ³n a pagar (AP)</span><span className="font-mono font-bold text-amber-700 text-base">{iauAP>0?fd(iauAP):'â€”'}</span></div>
                    </div>
                  </>}
                  {!iauActive&&<div className="px-3 py-4 text-center text-xs text-slate-400">Activa esta opciÃ³n para calcular la aportaciÃ³n y usar un IE superior al ampliado.</div>}
                </>}/>}
              </div>}
              {/* STEP 2 â€” Compra Terreno */}
              ...
              {st===3&&<div className="space-y-4">
                <Card t="Tiempos del Proyecto" icon={ic.clock} ch={<><In l="PlanificaciÃ³n" v={inp.planif} o={v=>u("planif",v)} sfx="m"/><In l="ConstrucciÃ³n" v={inp.const} o={v=>u("const",v)} sfx="m"/><In l="Post construcciÃ³n" v={inp.postConst} o={v=>u("postConst",v)} sfx="m"/><R l="DuraciÃ³n total" v={`${res?.dur} meses`} b a/></>}/>
                <Card t="Mix de Apartamentos" icon={ic.bldg} ch={<>
                  <div className="px-3 py-2 bg-slate-50 rounded-lg mb-3">
                    <div className="grid grid-cols-4 gap-3">
                      <div><div className="text-[10px] text-slate-400">Ãrea edificable</div><div className="text-sm font-bold">{f0(res?.aE)} mÂ²</div></div>
                      ...
                      <div><div className="text-[10px] text-emerald-600 font-semibold">Ãrea Aptos</div><div className="text-sm font-bold text-emerald-700">{f0(rentableArea)} mÂ²</div></div>
                    </div>
                    <div className="text-[9px] text-slate-400 mt-1">â† Calculado desde DistribuciÃ³n por Uso (pestaÃ±a Terreno)</div>
                  </div>
                  <Sub t="Modelos de apartamentos"/>
                  <div className="px-3 mb-2">
                    <div className="grid grid-cols-12 gap-1 text-[8px] font-bold text-slate-400 uppercase mb-1 px-1"><span className="col-span-2">Modelo</span><span className="col-span-1 text-center">mÂ²/apt</span><span className="col-span-1 text-center">% Rent</span><span className="col-span-2 text-center">mÂ² asig.</span><span className="col-span-1 text-center">{inp.mixAdjust?'Uds âœŽ':'Uds'}</span><span className="col-span-1 text-center">mÂ² real</span><span className="col-span-1 text-center">% mix</span><span className="col-span-2 text-center">Pk/apt</span><span className="col-span-1"/></div>
                    {mixCalc.map((mc,i)=>{
                      ...
                      return(<div key={mc.id||i}>
                        <div className="grid grid-cols-12 gap-1 items-center mb-1">
                          ...
                          <input type="number" className="col-span-1 px-1 py-1 border border-slate-200 rounded text-[10px] text-center" value={mc.m2||''} placeholder="mÂ²" onChange={e=>{const nm=[...mixMdls];nm[i]={...nm[i],m2:+e.target.value||0};u('mixModels',nm);}}/>
                          ...
                          <button className="col-span-1 text-slate-300 hover:text-red-500 text-center" onClick={()=>{const nm=mixMdls.filter((_,j)=>j!==i);u('mixModels',nm);}}>Ã—</button>
                        </div>
                      </div>);
                    })}
                    <div className="flex gap-2 mt-2">
                      ...
                      <button onClick={()=>{if(!mixMdls.length)return;const share=Math.round(100/mixMdls.length*10)/10;const nm=mixMdls.map(m=>({...m,pctRent:share}));u('mixModels',nm);}} className="px-3 py-2 border border-emerald-200 rounded-lg text-xs font-semibold text-emerald-600 bg-emerald-50 hover:bg-emerald-100">âŸ³ Auto %</button>
                    </div>
                  </div>
                  {mixMdls.length>0&&<>
                    {/* Summary */}
                    <div className="px-3 py-2 bg-slate-50 rounded-lg mx-3 mt-2">
                      <div className="flex justify-between text-xs mb-1"><span className="text-slate-500">% rentable asignado</span><span className={`font-bold font-mono ${Math.abs(mixTotalPctR-100)<0.5?'text-emerald-600':mixTotalPctR>100?'text-red-500':'text-amber-500'}`}>{mixTotalPctR.toFixed(1)}%{Math.abs(mixTotalPctR-100)<0.5?' âœ“':mixTotalPctR>100?' â€” excede 100%':''}</span></div>
                      <div className="flex justify-between text-xs mb-1"><span className="text-slate-500">mÂ² realmente utilizados</span><span className="font-bold font-mono">{f0(mixTotalM2Used)} / {f0(rentableArea)} mÂ²</span></div>
                      <div className="flex justify-between text-xs mb-1"><span className="text-slate-500">DÃ©ficit de Ã¡rea</span><span className={`font-bold font-mono ${Math.abs(mixDeficit)<10?'text-emerald-600':mixDeficit>0?'text-amber-500':'text-red-500'}`}>{Math.abs(mixDeficit)<10?'0 mÂ² âœ“':mixDeficit>0?f0(mixDeficit)+' mÂ² sin usar':f0(Math.abs(mixDeficit))+' mÂ² excedido'}</span></div>
                      <div className="flex justify-between text-xs mb-1"><span className="text-slate-500">Unidades totales</span><span className="font-bold text-emerald-600 text-base font-mono">{mixTotalUn||'â€”'}</span></div>
                    ...
                    <div className="px-3 mx-3 mt-2 py-2 bg-white rounded-lg border border-slate-200">
                      ...
                      {inp.mixAdjust&&<div className="text-[9px] text-amber-600 mb-2 pl-5">Edita las unidades directamente en la columna "Uds". Deja vacÃ­o para usar el cÃ¡lculo automÃ¡tico.</div>}
                      ...
                      <div className="text-[9px] text-slate-400 pl-5 mt-1">Ajusta el % vendible de aptos de {inp.pctVendApt}% a {mixAdjPctVend.toFixed(2)}% para reflejar el Ã¡rea realmente utilizada ({f0(mixTotalM2Used)} mÂ²) sobre el Ã¡rea edificable ({f0(res?.aE)} mÂ²).</div></>}
                    </div>
                    {/* Feeding indicator */}
                    <div className="px-3 py-2 bg-emerald-50 rounded-lg mx-3 mt-2 border border-emerald-200">
                      <div className="flex justify-between text-xs mb-1"><span className="text-emerald-700 font-semibold">â†’ Alimentando Producto</span></div>
                      <div className="flex justify-between text-[10px]"><span className="text-slate-500">mÂ² prom. ponderado</span><span className="font-bold text-emerald-700 font-mono">{mixPondM2>0?mixPondM2.toFixed(1)+' mÂ²':'â€”'}</span></div>
                      <div className="flex justify-between text-[10px]"><span className="text-slate-500">Pk/apto</span><span className="font-bold text-emerald-700 font-mono">{mixPkPerApt>0?mixPkPerApt.toFixed(3):'â€”'}</span></div>
                    </div>
                  </>}
                </>}/>
                ...
                <Card t="Apartamentos" icon={ic.bldg} ch={<>
                  {mixMdls.length>0&&<div className="mx-3 mb-2 px-3 py-2 bg-emerald-50 rounded-lg border border-emerald-200"><div className="text-[10px] text-emerald-600 font-semibold mb-1">â† Alimentado desde Mix Producto</div><div className="flex justify-between text-xs"><span className="text-slate-500">mÂ² ponderado</span><span className="font-mono font-bold text-emerald-600">{mixPondM2.toFixed(1)}</span></div><div className="flex justify-between text-xs"><span className="text-slate-500">Pk/apto</span><span className="font-mono font-bold text-emerald-600">{mixPkPerApt.toFixed(3)}</span></div></div>}
                  <In l="TamaÃ±o promedio" v={inp.tamApto} o={v=>u("tamApto",v)} sfx="mÂ²"/>
                  <In l="Parqueos por apto" v={inp.parqApto} o={v=>u("parqApto",v)} step="0.001"/>
                  <Sub t="Resultados"/>
                  <R l="Ãrea aptos" v={`${f0(res?.aApt)} mÂ²`}/>
                  <R l="NÃºmero de aptos" v={f0(res?.nApt)} b a/>
                  <R l="Parqueos aptos" v={f0(res?.pApt)}/>
                  <R l="Parqueos visitas" v={f0(res?.pVis)}/>
                </>}/>
                <Card t="Comercio y Oficinas" icon={ic.bldg} ch={<>
                  <Sub t="Comercio"/>
                  <R l="Ãrea comercio" v={`${f0(res?.aCom)} mÂ²`}/>
                  <In l="Parqueos comercio" v={inp.nPkCom} o={v=>u("nPkCom",v)}/>
                  <R l="mÂ² rentable/parqueo" v={res?.m2PerPkCom>0?f1(res.m2PerPkCom)+' mÂ²/pk':'â€”'}/>
                  <Sub t="Oficinas"/>
                  <R l="Ãrea oficinas" v={`${f0(res?.aOfi)} mÂ²`}/>
                  <In l="Unidades oficinas" v={inp.uniOfi} o={v=>u("uniOfi",v)}/>
                  <In l="Parqueos oficinas" v={inp.nPkOfi} o={v=>u("nPkOfi",v)}/>
                  <R l="mÂ² rentable/parqueo" v={res?.m2PerPkOfi>0?f1(res.m2PerPkOfi)+' mÂ²/pk':'â€”'}/>
                </>}/>
                <Card t="SÃ³tanos y ProtecciÃ³n" icon={ic.ruler} ch={<>
                  <Sub t="Eficiencia de sÃ³tanos"/>
                  <In l="mÂ² por parqueo (eficiencia sÃ³tano)" v={inp.efSot} o={v=>u("efSot",v)} sfx="mÂ²/pk"/>
                  <Sub t="ProtecciÃ³n"/>
                  ...
                  <Sub t="Resumen parqueos y sÃ³tanos"/>
                  ...
                  <R l="Ãrea sÃ³tanos" v={`${f0(res?.aSot)} mÂ²`}/>
                  <R l="Niveles sÃ³tano" v={f2(res?.cSot)} b a/>
                </>}/>
                </div>
              </div>}
              {/* STEP 4 â€” Ventas */}
              {st===4&&<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Card t="Ventas â€” Aptos" icon={ic.dollar} ch={<><In l="Mes inicio ventas" v={inp.mesIniVtas} o={v=>u("mesIniVtas",v)}/><In l="Ritmo (aptos/mes)" v={inp.ritmoVtas} o={v=>u("ritmoVtas",v)}/><In l="Reserva" v={inp.reserva} o={v=>u("reserva",v)} sfx="%" step="0.1"/><In l="Enganche" v={inp.engApt} o={v=>u("engApt",v)} sfx="%"/><In l="Precio $/mÂ²" v={inp.pvM2} o={v=>u("pvM2",v)} pfx="$"/><In l="Precio parqueo" v={inp.pvParq} o={v=>u("pvParq",v)} pfx="$"/><R l="Venta aptos" v={fd(res?.vAptSinImp)} b a bd/></>}/>
                <Card t="Comercio" icon={ic.bldg} ch={<><R l="Ãrea comercio" v={`${f0(res?.aCom)} mÂ²`}/><In l="Renta $/mÂ²/mes" v={inp.rentaM2} o={v=>u("rentaM2",v)} pfx="$"/><In l="OPEX $/mÂ²" v={inp.opexM2} o={v=>u("opexM2",v)} pfx="$"/><In l="Renta pq/mes" v={inp.rentaParq} o={v=>u("rentaParq",v)} pfx="$"/><In l="Costo pq/mes" v={inp.costoParq} o={v=>u("costoParq",v)} pfx="$"/><In l="Cap Rate" v={inp.capRate} o={v=>u("capRate",v)} sfx="%"/><Sub t="Resultados"/><R l="NOI" v={fd(res?.noi)} b a/><R l="Valor comercio" v={fd(res?.vComSinImp)} b a bd/></>}/>
                <Card t="Oficinas" icon={ic.bldg} ch={<><R l="Ãrea oficinas" v={`${f0(res?.aOfi)} mÂ²`}/><R l="Unidades" v={f0(res?.nOfi)}/><R l="mÂ² por unidad" v={res?.aOfiUni>0?f0(res.aOfiUni)+' mÂ²':'â€”'}/><In l="Ritmo ofi/mes" v={inp.ritmoOfi} o={v=>u("ritmoOfi",v)}/><In l="Enganche" v={inp.engOfi} o={v=>u("engOfi",v)} sfx="%"/><In l="Precio $/mÂ²" v={inp.pvOfiM2} o={v=>u("pvOfiM2",v)} pfx="$"/><In l="Precio parqueo" v={inp.pvParqOfi} o={v=>u("pvParqOfi",v)} pfx="$"/><Sub t="Resultados"/><R l="Venta oficinas" v={fd(res?.vOfiSinImp)} b a bd/></>}/>
              </div>}
              {/* STEP 5 â€” Costos */}
              {st===5&&<div className="space-y-4">
                <div className="flex items-center gap-2 bg-white rounded-xl p-1 border border-slate-200 w-fit">
                  ...
                  <button onClick={()=>u("costoMode","estimado")} className={`px-4 py-2 text-xs font-semibold rounded-lg transition-all ${inp.costoMode==="estimado"?"bg-emerald-500 text-white shadow-sm":"text-slate-400 hover:bg-slate-50"}`}>Estimado por mÂ²</button>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {inp.costoMode==="estimado"
                  ? <Card t="Costo Estimado" icon={ic.hammer} ch={<>
                      <div className="px-3 py-2 text-xs text-slate-400">Ingresa un costo global por mÂ² de construcciÃ³n (sin IVA)</div>
                      <In l="Costo por mÂ²" v={inp.costoM2Est} o={v=>u("costoM2Est",v)} pfx="$" step="1"/>
                      <R l="Ãrea de construcciÃ³n" v={`${f0(res?.aConst)} mÂ²`}/>
                  ...
                  : <Card t="ConstrucciÃ³n ($/mÂ²)" icon={ic.hammer} ch={<>{[["Acabados com/ofi","acabComOfi"],["Acabados amenid.","acabAmen"],["Acabados aptos","acabApt"],["Acabados sÃ³tanos","acabSot"],["ExcavaciÃ³n","excCosto"],["CimentaciÃ³n","ciment"],["Equipamiento","equip"],["ExtracciÃ³n","extrac"],["Inst. elÃ©ctricas","instElec"],["Inst. hidrosanit.","instHidro"],["Obra primaria","obraPrim"],["Obra secundaria","obraSec"],["Preliminar","prelim"],["Soil nailing","snCosto"],["Pilotes","pilCosto"]].map(([l,k])=><In key={k} l={l} v={inp[k]} o={v=>u(k,v)}/>)}<In l="Anticipo" v={inp.anticipo} o={v=>u("anticipo",v)} sfx="%"/><R l="Costo total (con IVA)" v={fd(res?.cConst)} b a bd/><R l="$/mÂ² total" v={res?.aConst>0?'$'+f0(res.cConstBase/res.aConst)+'/mÂ²':'â€”'} b/></>}/>
                }
                <Card t="Equipo y Mobiliario" icon={ic.hammer} ch={<><div className="px-3 py-1 text-xs text-slate-400">$/mÂ² construcciÃ³n</div>{[["Lobby","eqLobby","eqLobbyCost"],["Terraza","eqTerraza","eqTerrazaCost"],["ElectrodomÃ©sticos","eqElectro","eqElectroCost"],["Cocinas/Closets/BaÃ±os","eqCocinas","eqCocinasCost"],["CCTV","eqCCTV","eqCCTVCost"],["Espejos/Mamparas","eqEspejos","eqEspejosCost"],["JardinizaciÃ³n","eqJardin","eqJardinCost"],["SeÃ±alizaciÃ³n","eqSenal","eqSenalCost"],["Ornato/mejoras","eqOrnato","eqOrnatoCost"]].map(([l,kb,kc])=><Toggle key={kb} l={l} ck={inp[kb]} oc={v=>u(kb,v)} cost={inp[kc]} occ={v=>u(kc,v)}/>)}<R l="Costo total (con IVA)" v={fd(res?.cEquip)} b a bd/><R l="$/mÂ² total" v={res?.aConst>0?'$'+f0(res.cEquipBase/res.aConst)+'/mÂ²':'â€”'} b/></>}/>
                <Card t="Gastos Operativos" icon={ic.chart} ch={<><div className="px-3 py-1 text-xs text-slate-400">% sobre ventas sin impuestos</div>{[["Admin desarrollo","pctAdminDes","opAdminSI"],["Gastos tÃ©cnicos","pctGasTec","opTecSI"],["Gastos legales","pctGasLeg","opLegSI"],["Mercadeo","pctMerc","opMercSI"],["Admin ventas","pctAdmVtas","opAdmVSI"],["ComercializaciÃ³n","pctComz","opComzSI"]].map(([l,k,rk])=><div key={k} className="group flex items-center justify-between gap-2 py-2 px-3 rounded-lg hover:bg-emerald-50/40"><span className="text-xs text-slate-600 leading-tight min-w-0">{l}</span><div className="flex items-center gap-1 shrink-0"><input type="number" value={inp[k]} step="0.5" onChange={e=>u(k,parseFloat(e.target.value)||0)} className="w-14 px-1.5 py-1.5 text-xs font-semibold text-emerald-800 bg-white border border-slate-200 rounded-lg text-right focus:outline-none focus:ring-2 focus:ring-emerald-400/40"/><span className="text-[10px] text-slate-400 w-4">%</span><span className="text-[10px] font-mono text-slate-400 w-16 text-right">{res&&res[rk]?fd(res[rk]):''}</span></div></div>)}<In l="Licencias" v={inp.pctLic} o={v=>u("pctLic",v)} sfx="%" step="0.25"/>{iauActive&&iauAP>0&&<R l="+ Edif. aumentada (AP)" v={fd(iauAP)} a/>}<R l="Total licencias (sin IVA)" v={fd(res?.opLicSI)} b/><R l="Total con IVA" v={fd(res?.tOpEx)} b a bd/></>}/>
              </div></div>}
              {/* STEP 6 â€” Financiamiento */}
              {st===6&&<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Card t="Capital y Deuda" icon={ic.dollar} ch={<>
                  ...
                  {res&&res.suggestedCapital>0&&<div className={`mx-3 mb-2 px-3 py-2 rounded-lg border ${inp.capital>=res.suggestedCapital*0.95?'bg-emerald-50 border-emerald-200':inp.capital>=res.suggestedCapital*0.7?'bg-amber-50 border-amber-200':'bg-red-50 border-red-200'}`}>
                    <div className="text-[10px] font-semibold text-slate-500 mb-2">Capital estimado â€” fase de planificaciÃ³n ({inp.planif} meses)</div>
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-xs text-slate-500">Egresos en planificaciÃ³n</span>
                    ...
                    <div className="flex justify-between text-[10px]">
                      ...
                      <span className={`font-semibold ${inp.capital>=res.suggestedCapital*0.95?'text-emerald-600':inp.capital>=res.suggestedCapital*0.7?'text-amber-600':'text-red-600'}`}>
                        {inp.capital>=res.suggestedCapital*0.95?'âœ“ Suficiente':inp.capital>=res.suggestedCapital*0.7?'âš  Ajustado':'âœ• Insuficiente'}
                      </span>
                    </div>
                    <div className="text-[9px] text-slate-400 mt-1">DÃ©ficit neto antes del primer desembolso bancario (mes {inp.planif+1}, inicio construcciÃ³n).</div>
                  </div>}
                  <In l="Tasa interÃ©s" v={inp.tasaInt} o={v=>u("tasaInt",v)} sfx="%" step="0.25"/>
                  ...
                  {res?.isAportado&&<R l="Terreno (aportaciÃ³n)" v={fd(res?.terrFundUsed)} b/>}
                  <R l="PrÃ©stamo bancario" v={fd(res?.prest)}/>
                  <R l="Total fuentes" v={fd((res?.capUsed||0)+(res?.terrFundUsed||0)+(res?.prest||0))} b a bd/>
                  <R l="Intereses totales" v={fd(res?.totInt)}/>
                </>}/>
                <div className="space-y-4">
                  <Card t="Resumen de Costos" icon={ic.chart} ch={<><R l="Terreno" v={fd(res?.cTerr)}/><R l="ConstrucciÃ³n" v={fd(res?.cConst)}/><R l="Equipo" v={fd(res?.cEquip)}/><R l="Gastos op." v={fd(res?.tOpEx)}/><R l="TOTAL" v={fd(res?.tCostos)} b a bd/><R l="Intereses" v={fd(res?.totInt)}/><R l="Impuestos" v={fd(res?.totalTax)}/></>}/>
              ...
              {st===7&&res&&<div className="space-y-4">
                ...
                <Card t="Estado de Resultados" icon={ic.chart} ch={<>
                  <div className="px-3 py-1 mb-1 flex items-center justify-between"><span className="text-[10px] bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full font-medium">Sin impuestos Â· Sin IVA</span><span className="text-[10px] text-slate-400 font-mono">Monto / % Ventas</span></div>
                  <PL l="Ventas" amt={res.vProy} vtas={res.vProy} b/>
                  <PL l="(-) ConstrucciÃ³n" amt={-res.cConstBase} vtas={res.vProy}/>
                  ...
                  <PL l="(-) Gastos tÃ©cnicos" amt={-res.opTecSI} vtas={res.vProy}/>
                  ...
                  <PL l="(-) ComercializaciÃ³n" amt={-res.opComzSI} vtas={res.vProy}/>
                ...
                <div className="space-y-4">
                  ...
                  <Card t="Costos Flujo de Caja" icon={ic.hammer} ch={<><R l="Terreno" v={fd(res.cTerr)}/><R l="ConstrucciÃ³n" v={fd(res.cConst)}/><R l="Equipo" v={fd(res.cEquip)}/><R l="Gastos op." v={fd(res.tOpEx)}/><R l="Timbres" v={fd(res.timbres)}/><R l="TOTAL" v={fd(res.tCostos)} b a bd/></>}/>
                </div>
                </div>
                {/* Charts â€” full width */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Waterfall Chart â€” P&L */}
                <Card t="Cascada de Rentabilidad" icon={ic.chart} ch={<>
                  {(()=>{
                    const steps=[
                      {l:'Ventas',v:res.vProy,type:'total'},
                      {l:'ConstrucciÃ³n',v:-res.cConstBase-res.cEquipBase},
              ...
              {st===8&&<div className="space-y-4">
                <Card t="ParÃ¡metros de Financiamiento" icon={ic.dollar} ch={<>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 px-3 py-2">
                    <div><div className="text-[10px] text-slate-500 mb-1">Enganche (%)</div><div className="px-3 py-1.5 bg-slate-50 rounded-lg text-xs font-bold text-slate-700 text-center">{inp.engApt}%<div className="text-[9px] text-slate-400 font-normal">â† Ventas</div></div></div>
                ...
                <Card t="AnÃ¡lisis por Modelo" icon={ic.bldg} ch={<>
                  {(()=>{
                    const mdls=mixCalc.filter(m=>m.m2>0&&m.units>0);
                    if(!mdls.length) return <div className="px-3 py-4 text-center text-xs text-slate-400">Agrega modelos en Mix Producto para ver el anÃ¡lisis de cuotas.</div>;
                        ...
                        {mdls.map((m,i)=>{
                          ...
                          return(<div key={m.id||i} className="bg-slate-50 rounded-xl border border-slate-200 p-3">
                            ...
                            <div className="space-y-1 text-[10px]">
                              <div className="flex justify-between"><span className="text-slate-400">TamaÃ±o</span><span className="font-mono font-bold">{m.m2} mÂ²</span></div>
            ...
            <div className="lg:col-span-4"><div className="sticky top-20 space-y-4">
              <div className="bg-slate-900 rounded-2xl p-5 text-white shadow-xl">
                <div className="text-xs text-slate-400 font-bold tracking-widest uppercase mb-3">MÃ©tricas Clave</div>
                <div className="grid grid-cols-2 gap-2.5">
                  <MC l="TIR antes imp." v={res?.airrBT?fp(res.airrBT):"â€”"} st={res?.airrBT!=null?iS(res.airrBT):null}/>
                  <MC l="TIR despuÃ©s imp." v={res?.airrAT?fp(res.airrAT):"â€”"} st={res?.airrAT!=null?iS(res.airrAT):null}/>
                  <MC l="MOIC antes imp." v={res?.moicBT?f2(res.moicBT)+"x":"â€”"} st={res?.moicBT!=null?mS(res.moicBT):null}/>
                  <MC l="MOIC despuÃ©s imp." v={res?.moicAT?f2(res.moicAT)+"x":"â€”"} st={res?.moicAT!=null?mS(res.moicAT):null}/>
              ...
              <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
                ...
                <R l="Ãrea terreno" v={`${f0(res?.aT*1.43115)} vÂ²`}/>
                <R l="Ãrea edificable" v={`${f0(res?.aE)} mÂ²`}/>
                <R l="Ãrea vendible" v={`${f0(res?.totalVend)} mÂ²`}/>
                <R l="Ãrea sÃ³tanos" v={`${f0(res?.aSot)} mÂ²`}/>
                <R l="Ãrea construcciÃ³n" v={`${f0(res?.aConst)} mÂ²`}/>
                <R l="SÃ³tanos" v={f2(res?.cSot)}/>
                {res?.niveles>0&&<R l="Niveles" v={f1(res?.niveles)}/>}
                {res?.alturaTotal>0&&<R l="Altura" v={`${f1(res?.alturaTotal)} m`}/>}
                <R l="DuraciÃ³n" v={`${res?.dur||"â€”"} meses`} bd/>
                <R l="Valor del proyecto" v={fd(res?.vProy)} b a/>
                <R l="EBIT" v={fp(res?.mEBIT)} b a/>
                <R l="Capital total" v={fd(res?.tCap)}/>
                <R l="PrÃ©stamo" v={fd(res?.prest)}/>
              </div>
...
const REF_CONST_LABELS={acabAmen:"Acabados amenidades",acabApt:"Acabados apartamentos",acabSot:"Acabados sÃ³tanos",ciment:"CimentaciÃ³n",equip:"Equipamiento obra",exc:"ExcavaciÃ³n",extrac:"ExtracciÃ³n",instElec:"Inst. elÃ©ctricas",instHidro:"Inst. hidrosanitarias",obraPrim:"Obra primaria",obraSec:"Obra secundaria",prelim:"Preliminar",protPilotes:"ProtecciÃ³n pilotes",protSN:"ProtecciÃ³n soil nailing"};
const REF_EQUIP_LABELS={eqLobby:"Lobby",eqTerraza:"Terraza",eqElectro:"ElectrodomÃ©sticos",eqCocinas:"Cocinas/closets/baÃ±os",eqCCTV:"CCTV",eqEspejos:"Espejos y mamparas",eqJardin:"JardinizaciÃ³n",eqSenal:"SeÃ±alizaciÃ³n",eqOrnato:"Ornato y mejoras",eqImprevistos:"Imprevistos"};
const REF_TECH_LABELS={suelos:"Suelos",topografia:"TopografÃ­a",arquitectura:"Arquitectura",estructural:"Estructural",hidraulico:"HidrÃ¡ulico y mecÃ¡nico",ptratamiento:"Planta tratamiento",electrico:"ElÃ©ctrico",supervision:"SupervisiÃ³n tÃ©cnica",consultoria:"ConsultorÃ­a varias",imprevistos:"Imprevistos",piscina:"Piscina"};
const REF_CHAR_LABELS={constructora:"Constructora",year:"AÃ±o",uso:"Uso",ubicacion:"UbicaciÃ³n",sotanos:"SÃ³tanos",niveles:"Niveles",unidades:"Unidades",torres:"Torres",aptosPorNivel:"Aptos/nivel",m2Const:"mÂ² construcciÃ³n",m2Planta:"mÂ² planta tÃ­pica",altura:"Altura",parqueos:"Parqueos",tiempoEjec:"Tiempo ejecuciÃ³n"};
const REF_DATA=[
...
{name:"SALOMÃ‰",categoria:"C-",construccion:{acabAmen:740.8,acabApt:286.44,acabSot:6.98,ciment:32.58,equip:10.11,exc:10.0,extrac:11.74,instElec:26.94,instHidro:25.09,obraPrim:169.61,obraSec:46.12,prelim:15.02,total:527.5},equipo:{},tecnico:{},caracteristicas:{constructora:"MACRO",year:2023,uso:"RESIDENCIAL",ubicacion:"ZONA 1",sotanos:8,niveles:12,unidades:250,torres:1,aptosPorNivel:8,m2Const:"29,133 m2",m2Planta:"1,372 m2",tiempoEjec:"22 meses"}},
