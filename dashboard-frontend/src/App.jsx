...
// Dashboard lives in its own component so all hooks are unconditional
    ...
    <>
      <nav className="nav-tabs">
        <div className="nav-brand">
          <span className="nav-brand-intus">INTUS</span>
          <span className="nav-brand-capital"> CAPITAL</span>
        </div>
        <div className="nav-tabs-center">
          ...
          <button
            className={`nav-tab${activeTab === 'market-intel' ? ' tab-active' : ''}`}
        ...
        <div className="nav-tabs-right">
          ...
          <button className="nav-signout" onClick={signOut}>Salir</button>
        </div>
      </nav>
      ...
      {/* Kept mounted (CSS-hidden) rather than unmounted on tab switch so
          in-progress work in Intel. de Mercado — like a PDF extraction or
          unreviewed proposed rows — survives navigating to another tab. */}
      <div style={{ display: activeTab === 'market-intel' ? 'block' : 'none' }}>
        <MarketIntelPanel />
      </div>
...
export default function App() {
  ...
  if (session === null)      return <LoginPage />;  // not logged in
